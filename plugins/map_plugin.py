...
class MapPlugin(Plugin):
    async def process(self, cleaned_lines):
        ...
        maps_yaml = await map_parser.insert_or_update_maps_async(False, folder, packet_list)
        fc_maps_yaml = await map_parser.insert_or_update_maps_async(True, folder, packet_list)
        ...
        with open(os.path.join(output_maps_dir, "act4_maps.yaml"), "w") as yaml_file:
            yaml_file.write(fc_maps_yaml)
